...
module Data.SBV.Control.Query (
     ...
     , inNewAssertionStack, echo, caseSplit, resetAssertions, exit, getAssertions, getModel, getSMTResult, getSMTResultUsing
...
getSMTResultUsing :: (MonadIO m, MonadQuery m) => String -> m SMTResult
getSMTResultUsing using = do
    cfg <- getConfig
    cs  <- checkSatUsing using
    case cs of
      Unsat  -> Unsatisfiable cfg   <$> getUnsatCoreIfRequested
      Sat    -> Satisfiable   cfg   <$> getModel
      DSat p -> DeltaSat      cfg p <$> getModel
