...
module Data.SBV.Internals (
  ...
  , quantVar, slet, SymbolicT(..), uninterpretedFun, interpretedFun

  -- * Is this name reserved?
  , isReserved, UIName(..)

  -- * Operations useful for instantiating SBV type classes
  , genLiteral, genFromCV, CV(..), genMkSymVar, genParse, showModel, SMTModel(..), liftQRem, liftDMod, registerKind, svToSV, svToSymSV
...
import Data.SBV.Core.Model      (uninterpretedFun, interpretedFun, slet, genLiteral, genFromCV, genMkSymVar, liftQRem, liftDMod)
import Data.SBV.Core.Symbolic   (SymbolicT(..), quantVar, IStage(..), QueryContext(..), MonadQuery, addSValOptGoal, registerKind, VarContext(..), svToSV, mkNewState, UICodeKind(..), UIName(..), svToSymSV)
...
import Data.SBV.Provers.Prover (runWithQuery, ProvableM, SatisfiableM)
