...
lambdaGen scope trans inState fk f = lambdaGenAux scope trans inState fk (\st -> mkLambda st f)

lambdaGenAux :: (MonadIO m) => LambdaScope -> (Defn -> b) -> State -> Kind -> (State -> SymbolicT m ()) -> m b
lambdaGenAux scope trans inState fk mk = inSubState scope inState $ \st -> handle <$> convert st fk (mk st)
...
lambdaDynamic :: (MonadIO m) => State -> LambdaScope -> Kind -> [Kind] -> ([SVal] -> SymbolicT m SVal) -> m SMTDef
lambdaDynamic inState scope fk kins f = lambdaGenAux scope mkLam inState fk (\st -> mkDynamicLambda st kins f)
   where mkLam (Defn unints _frees params body) = SMTDef fk unints (extractAllUniversals <$> params) body
