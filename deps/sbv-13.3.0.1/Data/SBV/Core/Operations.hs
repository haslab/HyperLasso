...
module Data.SBV.Core.Operations
  ...
  -- Utils
  , mkSymOp
  , svReadArray, svWriteArray, svListArray, svConstArray
  )
  where
...
svUnliteral :: SVal -> Maybe CV
svUnliteral (SVal _ (Left c)) = Just c
svUnliteral _ = Nothing

svLiteral :: CV -> SVal
svLiteral cv@(CV k _) = SVal k (Left cv)

svReadArray :: SVal -> SVal -> SVal
svReadArray array key
   | eqCheckIsObjectEq ka, Just (CV _ (CArray (ArrayModel tbl def))) <- svUnliteral array, Just (CV _ k) <- svUnliteral key
   = svLiteral $ CV kb $ fromMaybe def (k `lookup` tbl) -- return the first value, since we don't bother deleting previous writes
   | True
   = symRes
   where symRes = SVal kb . Right $ cache g
         (ka,kb) = case kindOf array of
             KArray ka_ kb_ -> (ka_,kb_)
             _ -> error "svReadArray not an array"
         g st = do f <- svToSV st array
                   k <- svToSV st key
                   newExpr st kb (SBVApp ReadArray [f, k])

svWriteArray :: SVal -> SVal -> SVal -> SVal
svWriteArray array key value
    | Just (CV _ (CArray (ArrayModel tbl def))) <- svUnliteral array, Just (CV _ keyVal) <- svUnliteral key, Just (CV _ val) <- svUnliteral value
    = svLiteral $ CV k $ CArray $ ArrayModel ((keyVal, val) : tbl) def  -- It's important that we "cons" the value here, since it takes precedence in a read
    | True
    = SVal k . Right $ cache g
  where
    k = KArray (kindOf key) (kindOf value)
    g st = do
        arr <- svToSV st array
        keyVal <- svToSV st key
        val <- svToSV st value
        newExpr st k (SBVApp WriteArray [arr, keyVal, val])

svListArray :: [(CVal,CVal)] -> Kind -> CV -> SVal
svListArray ascs ka def@(CV _ defval) = svLiteral $ CV karr $ CArray $ ArrayModel ascs defval
    where
    kb = kindOf def
    karr = KArray ka kb

svConstArray :: Kind -> SVal -> SVal
svConstArray ka v
    | Just (CV _ v') <- svUnliteral v
    = svLiteral $ CV k $ CArray $ ArrayModel [] v'
    | True
    = SVal k . Right $ cache g
  where
    kb = kindOf v
