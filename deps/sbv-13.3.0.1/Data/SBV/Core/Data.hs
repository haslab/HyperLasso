...
mkDynamicLambda :: MonadSymbolic m => State -> [Kind] -> ([SVal] -> m SVal) -> m ()
mkDynamicLambda st kins f = cps kins f 
    where
    cps :: MonadSymbolic m => [Kind] -> ([SVal] -> m SVal) -> m ()
    cps [] g = void $ outputSVal =<< g []
    cps (k:ks) g = do
        a <- mkArg k
        cps ks (g . (a:))
    mkArg :: MonadSymbolic m => Kind -> m SVal
    mkArg k = do
        sv <- liftIO $ lambdaVar st k
        pure $ SVal k (Right (cache (const (return sv))))
