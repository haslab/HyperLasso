...
addModuleTypes p = p
    ...
    , p_ltlspec = (addExprTypes ss) (p_ltlspec p)
    }  
  where
...
addFormulaTypes = addFormulaTypes' Map.empty
    ...
    addFormulaTypes' acc ss f = error $ "addFormulaTypes " ++ unwords (map show ss) ++ " " ++ prettyprint f
...
varTypeOfPop2 Pmod (VInt is) (VInt js) = if IntSet.null js
    then VInt IntSet.empty 
    else let j = IntSet.findMax js in VInt $ IntSet.filter (\i -> 0 <= i && i < j) is
...
isIntExpr :: Pexpr -> Bool
isIntExpr e = typeOfExpr e == EInt
...
type VarTypes = Map Pident VarType