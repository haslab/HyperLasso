...
intersectionsIntSet :: [IntSet] -> IntSet
intersectionsIntSet [] = IntSet.empty
intersectionsIntSet [x] = x
intersectionsIntSet (x:xs) = IntSet.intersection x (intersectionsIntSet xs)
...
modifyAssoc :: Eq k => k -> (v -> v) -> [(k,v)] -> [(k,v)]
modifyAssoc k f [] = []
modifyAssoc k f ((xk,xv):xs) = if k==xk then (xk,f xv) : xs else (xk,xv) : modifyAssoc k f xs
...
multiMapKeysSet :: MultiMap a b -> Set a
multiMapKeysSet (MultiMap xs) = Map.keysSet xs
...
nextExponentOf2 :: Int -> Int
nextExponentOf2 n
  | n <= 1    = 0
  | otherwise = ceiling (logBase 2 (fromIntegral n :: Double))

productMaps :: Ord k => Map k a -> Map k b -> Map k (a,b)
productMaps = Map.merge Map.dropMissing Map.dropMissing (Map.zipWithMaybeMatched $ \k a b -> Just (a,b))

joinMapWithKey :: Ord k => (k -> a -> b -> c) -> Map k a -> Map k b -> Map k c
joinMapWithKey f = Map.merge Map.dropMissing Map.dropMissing (Map.zipWithMaybeMatched $ \k a b -> Just $ f k a b )

joinMap :: Ord k => (a -> b -> c) -> Map k a -> Map k b -> Map k c
joinMap f = joinMapWithKey (const f)

zipMap :: Ord k => Map k a -> Map k b -> Map k (a,b)
zipMap = joinMap (,)
    
lcms :: (Integral a,Foldable t) => t a -> a
lcms = foldl1 lcm
    
toBits :: Int -> [Bool]
toBits i = map Prelude.toEnum $ digits 2 i

fromBits :: [Bool] -> Int
fromBits bs = unDigits 2 (map Prelude.fromEnum bs)

mapHead :: (a -> a) -> [a] -> [a]
mapHead f [] = []
mapHead f (x:xs) = f x : xs

mapUnionError :: Ord a => Map a b -> Map a b -> Map a b
mapUnionError = Map.unionWith $ error "mapUnionError: duplicated keys unexpected"
...
allEqual [] = Nothing
allEqual [x] = Just x
allEqual (x:y:xs) = if x==y then allEqual (y:xs) else Nothing
...
measureTime m = do
    ...
    return (res,diffUTCTime end start)

measureTime' :: (NFData a,MonadIO m) => m a -> m (a,NominalDiffTime)
measureTime' m = do
    start <- liftIO $ getCurrentTime
    res <- m
    end <- rnf res `seq` liftIO $ getCurrentTime
    return $! (res,diffUTCTime end start)

type TimeRef = IORef NominalDiffTime

measureTimeRef :: MonadIO m => TimeRef -> m a -> m a
measureTimeRef timeRef m = do
    (a,time) <- measureTime m
    liftIO $ atomicModifyIORef timeRef (\t -> (t+time,()) )
    return a

measureTimeRef' :: (NFData a,MonadIO m) => TimeRef -> m a -> m a
measureTimeRef' timeRef m = do
    (a,time) <- measureTime' m
    liftIO $ atomicModifyIORef' timeRef (\t -> (t+time,()) )
    return a

maybeMeasureTimeRef' :: (NFData a,MonadIO m) => Maybe TimeRef -> m a -> m a
maybeMeasureTimeRef' Nothing m = m
maybeMeasureTimeRef' (Just timeRef) m = measureTimeRef' timeRef m

showTime :: NominalDiffTime -> String
showTime t = printf "%.4fs" (realToFrac t :: Double)

instance Pretty NominalDiffTime where
    pretty t = pretty (showTime t)
